...
class CrawlerDev::Test::SingletonClient < CrawlerDev::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: CrawlerDev::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class CrawlerDev::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @crawler_dev = CrawlerDev::Test::SingletonClient.instance
  end
