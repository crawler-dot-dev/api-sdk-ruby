...
class CrawlerDevTest < Minitest::Test
  ...
  def test_raises_on_missing_non_nullable_opts
    e = assert_raises(ArgumentError) do
      CrawlerDev::Client.new
    end
    assert_match(/is required/, e.message)
  end

  def test_client_default_request_default_retry_attempts
    stub_request(:post, "http://localhost/v1/files/text").to_return_json(status: 500, body: {})

    crawler_dev = CrawlerDev::Client.new(base_url: "http://localhost", api_key: "My API Key")

    assert_raises(CrawlerDev::Errors::InternalServerError) do
      crawler_dev.files.extract_text(file: Pathname(__FILE__))
    end
  ...
  def test_client_given_request_default_retry_attempts
    ...
    crawler_dev = CrawlerDev::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 3)
  ...
  def test_client_default_request_given_retry_attempts
    ...
    assert_raises(CrawlerDev::Errors::InternalServerError) do
      crawler_dev.files.extract_text(file: Pathname(__FILE__), request_options: {max_retries: 3})
    end
  ...
  def test_client_given_request_given_retry_attempts
    ...
    assert_raises(CrawlerDev::Errors::InternalServerError) do
      crawler_dev.files.extract_text(file: Pathname(__FILE__), request_options: {max_retries: 4})
    end
  ...
  def test_client_retry_after_seconds
    stub_request(:post, "http://localhost/v1/files/text").to_return_json(
      status: 500,
      headers: {"retry-after" => "1.3"},
      body: {}
    )

    crawler_dev = CrawlerDev::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(CrawlerDev::Errors::APIConnectionError) do
      crawler_dev.files.extract_text(file: Pathname(__FILE__), request_options: {extra_headers: {}})
    end

    recorded, = WebMock::RequestRegistry.instance.requested_signatures.hash.first

    assert_requested(:any, "http://localhost/redirected", times: CrawlerDev::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: CrawlerDev::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: CrawlerDev::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:post, "http://localhost/v1/files/text").to_return_json(status: 200, body: {})
